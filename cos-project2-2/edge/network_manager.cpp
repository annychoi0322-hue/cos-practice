...
int NetworkManager::sendData(uint8_t *data, int dlen)
{
  ...
  // Send 1-byte opcode first.
  // OPCODE_DATA tells the server that the following bytes are processed feature data.
  opcode = OPCODE_DATA;
  tbs = 1;
  offset = 0;
  ...
  // Send the processed feature vector generated by ProcessManager::processData().
  // The byte length is not fixed because each feature version has a different layout.
  // V1: [avg_power, avg_temp, avg_humid, month] = 5 bytes
  // V2: [avg_power, avg_temp, temp_range, avg_humid, month] = 6 bytes
  // V3: [avg_power, max_temp, min_temp, avg_humid, month] = 6 bytes
  tbs = dlen;
  offset = 0;
