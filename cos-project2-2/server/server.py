...
class Server:
    def __init__(self, name, algorithm, dimension, index, port, caddr, cport, ntrain, ntest):
        ...
        # ============================================================
        # Select the byte length of the active feature version.
        #
        # V1: [avg_power, avg_temp, avg_humid, month]
        #     dimension = 4, byte length = 5
        #
        # V2: [avg_power, avg_temp, temp_range, avg_humid, month]
        #     dimension = 5, byte length = 6
        #
        # V3: [avg_power, max_temp, min_temp, avg_humid, month]
        #     dimension = 5, byte length = 6
        #
        # This value must match the byte layout generated by
        # edge/process_manager.cpp.
        # ============================================================

        # self.data_len = 5      # Use this for V1
        self.data_len = 6    # Use this for V2 or V3
    ...
    def parse_data(self, buf, is_training):
        ...
        # target index = 0
        #
        # byte layout from edge/process_manager.cpp:
        # buf[0:2] = avg_power
        # buf[2:3] = avg_temp
        # buf[3:4] = avg_humid
        # buf[4:5] = month
        ...
        power = int.from_bytes(buf[0:2], byteorder="big", signed=True)
        temp = int.from_bytes(buf[2:3], byteorder="big", signed=True)
        temp_range = int.from_bytes(buf[3:4], byteorder="big", signed=True)
        humid = int.from_bytes(buf[4:5], byteorder="big", signed=True)
        month = int.from_bytes(buf[5:6], byteorder="big", signed=True)

        lst = [power, temp, temp_range, humid, month]
        logging.info("[avg_power, avg_temp, temp_range, avg_humid, month] = {}".format(lst))
        """
        ...
        power = int.from_bytes(buf[0:2], byteorder="big", signed=True)
        max_temp = int.from_bytes(buf[2:3], byteorder="big", signed=True)
        min_temp = int.from_bytes(buf[3:4], byteorder="big", signed=True)
        humid = int.from_bytes(buf[4:5], byteorder="big", signed=True)
        month = int.from_bytes(buf[5:6], byteorder="big", signed=True)

        lst = [power, max_temp, min_temp, humid, month]
    ...
    def handler(self, client):
        ...
        while True:
            # Receive opcode from the edge device.
            # OPCODE_DATA means that the next bytes are processed feature data.
            rbuf = client.recv(1)
            ...
            if opcode == OPCODE_DATA:
                ...
                # Receive the processed feature vector.
                # The byte length depends on the active feature version.
                rbuf = client.recv(self.data_len)
